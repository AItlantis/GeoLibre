...
export function setScenarioComparisonSettings(next:Partial<ScenarioComparisonSettings>){const oldMode=settings.mode;settings={...settings,...next};if(provider&&(next.scenarioA!==undefined||next.scenarioB!==undefined))updateReplications(provider);if(settings.mode!==oldMode)(settings.mode==="side-by-side"?enableSideBySide:disableSideBySide)();if(next.metric!==undefined&&settings.mode==="side-by-side"){disableSideBySide();enableSideBySide();}if(next.interval !== undefined || next.didA !== undefined || next.didB !== undefined || next.scenarioA !== undefined || next.scenarioB !== undefined) void readCurrent(); if(layerVisible)render();else setSharedDeckLayers("scenario-comparison",[]);notify();}
