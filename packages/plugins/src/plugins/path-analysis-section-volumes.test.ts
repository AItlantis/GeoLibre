...
test("aggregates unique route demand by section and normalizes to the selected section", () => {
  ...
  assert.deepEqual(result.sections, [
    { section_id: 1, volume: 100, percentage: 100, role: "upstream" },
    { section_id: 2, volume: 100, percentage: 100, role: "selected" },
    { section_id: 3, volume: 60, percentage: 60, role: "downstream" },
    { section_id: 4, volume: 40, percentage: 40, role: "downstream" },
  ]);
});
...
test("deduplicates route ids and duplicate route-link occurrences", () => {
  ...
  assert.deepEqual(result.sections, [
    { section_id: 5, volume: 25, percentage: 100, role: "selected" },
    { section_id: 6, volume: 25, percentage: 100, role: "downstream" },
  ]);
});
